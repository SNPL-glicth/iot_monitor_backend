...
/**
 * SensorReadingQueryService — Lecturas de sensores y métricas agregadas.
 *
 * Incluye insertSensorReading (escritura vía SP) por ser parte del dominio
 * de lecturas/telemetría.
 */
@Injectable()
export class SensorReadingQueryService {
  constructor(
  ...
  /**
   * Datos crudos del sensor SIN agregación.
   */
  ...
  /**
   * Lecturas históricas por rango de fechas ABSOLUTAS.
   */
  async getHistoricalReadings(
    ...
    const thresholdData = {
      alertMin: alertThreshold?.thresholdValueMin
        ? Number(alertThreshold.thresholdValueMin)
        : null,
      alertMax: alertThreshold?.thresholdValueMax
        ? Number(alertThreshold.thresholdValueMax)
        : null,
